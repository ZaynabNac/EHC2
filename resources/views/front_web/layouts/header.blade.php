                    ...
                    <div class="collapse navbar-collapse justify-content-end" id="navbarNav">
                        <ul class="navbar-nav align-items-center py-2 py-lg-0">
                            ...
                            <li class="nav-item">
                                ...
                                @foreach(\App\Models\Language::where('is_default', 1)->pluck('language', 'iso_code') as $key => $value)
    <li class="languageSelection {{ (checkLanguageSession() == $key) ? 'active' : '' }}" data-prefix-value="{{ $key }}" style="max-height: 40px">
        <a class="dropdown-item {{ (checkLanguageSession() == $key) ? 'active' : '' }}" href="javascript:void(0)">
            @if(array_key_exists($key, \App\Models\User::LANGUAGES_IMAGE))
                @foreach(\App\Models\User::LANGUAGES_IMAGE as $imageKey => $imageValue)
                    @if($imageKey == $key)
                        <img class="me-2 country-flag" style="width: 20px;" src="{{ asset($imageValue) }}" />
                    @endif
                @endforeach
            @else
                <i class="fa fa-flag me-2 fs-7 text-danger" aria-hidden="true" style="width: 20px;"></i>
            @endif
            {{ $value }}
        </a>
    </li>
